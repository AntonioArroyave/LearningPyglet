...
class surface:
    ...
    def __init__(self, number_of_squares = (4), number_of_dimensions = (2), length=(1)):
        ...
        Theta = np.arange(0,pi+step,step)
        Phi = np.arange(0,2*pi+step,step)
        radio = length
        #V = [x for x in np.arange(-length/2, (length/2)+self.step, self.step)]
        V = [[radio*sin(theta)*cos(phi), radio*sin(theta)*sin(phi), radio*cos(theta)] for theta in Theta for phi in Phi] # here must be replace for de desired function of z 
...
def main():
    ...
    while True:
        ...
        hiperbolic = surface(number_of_squares = (1), number_of_dimensions = (2), length=(1))
